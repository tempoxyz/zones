...
| `TEMPO-ZONE-WITHDRAWAL-FEE-NONBLOCKING` | A failed sequencer fee transfer never reverts `processWithdrawal`; the withdrawal still completes and the sequencer keeps the fee only when its transfer succeeds | 🟡 | A fee-transfer failure can stall the withdrawal queue or block exits |
