...
| `TEMPO-ZONE-WITHDRAWAL-CALLBACK-RETURNDATA-BOUND` | Realized delivery cost of a callback withdrawal stays within its `gasLimit` plus a fixed overhead, because `relayMessage`'s return and revert data are bounded by a small constant for every callback target. Bounded for *any* callee additionally assumes enabled tokens are factory-minted TIP-20s whose `transfer` reverts with constant-size errors, which `enableToken` enforces via `isTIP20`; a token with arbitrary code could both bubble oversized revert data and burn the uncapped gas the transfer call forwards | 🟡 | A callback can charge unbounded quadratic memory gas to the delivery frame, exhausting a `processWithdrawals` batch and permanently blocking the FIFO queue |
