...
| `TEMPO-ZONE-WITHDRAWAL-CALLBACK-RETURNDATA-BOUND` | `ZoneMessenger` never copies a callback target's response or revert data into its own frame beyond the single word a `bytes4` return occupies, so realized delivery cost stays within `gasLimit` plus fixed overhead | 🟡 | A callback can charge unbounded quadratic memory gas to the delivery frame, exhausting a `processWithdrawals` batch and permanently blocking the FIFO queue |
