...
use crate::{ZoneManifest, manifest::AuthoritySnapshot};
...
impl<'a> RoutingPolicy<'a> {
    ...
    pub(crate) fn may_accept_settlement_proposal(&self, peer: &PublicKey) -> bool {
        self.membership.is_quorum_member(self.local) && self.is_remote_retained_leader(peer)
    }
    ...
    pub(crate) fn may_accept_settlement_signature(&self, peer: &PublicKey) -> bool {
        self.is_remote_quorum_peer(peer) && self.may_broadcast_block()
    }
    ...
    pub(crate) fn may_forward_transaction(&self) -> bool {
        self.authority
            .next_anchor_record
            .as_ref()
            .is_some_and(|record| record.leader != *self.local)
    }
...
mod tests {
    ...
    fn routing_policy_covers_roles_and_transaction_fanout() {
        ...
        let authority = schedule.authority_snapshot();
        ...
        assert_eq!(
            leader
                .settlement_proposal_recipients()
                .into_iter()
                .collect::<BTreeSet<_>>(),
