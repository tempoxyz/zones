...
contract ForcedExitVectorsTest is Test {
    ...
    function test_solidityRustCommitments() public view {
        ...
        bytes32 tag = keccak256(abi.encode("TempoZoneForcedExit", portal, uint64(1)));
        ...
        ForcedExitOutcome[] memory outcomes = new ForcedExitOutcome[](1);
        outcomes[0] = ForcedExitOutcome(
            1, 2, token, recipient, ForcedExitStatus.Exited, 0, 123_456, withdrawalHash
        );
        assertEq(keccak256(abi.encode(outcomes)), vm.parseJsonBytes32(fixture, ".outcomesHash"));
        assertEq(
            keccak256(abi.encode(new ForcedExitOutcome[](0))),
            vm.parseJsonBytes32(fixture, ".emptyOutcomesHash")
        );
        ...
        assertEq(uint8(ForcedExitStatus.Exited), 0);
        assertEq(uint8(ForcedExitStatus.Empty), 1);
        assertEq(uint8(ForcedExitStatus.Rejected), 2);
