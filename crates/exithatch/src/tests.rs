...
#[test]
fn outcome_shapes_and_commitments() {
    let mut outcome = ForcedExitOutcome {
        requestId: 1,
        depositNumber: 2,
        token: TOKEN,
        recipient: Address::ZERO,
        status: ForcedExitStatus::Empty,
        reason: 0,
        amount: 0,
        withdrawalHash: B256::ZERO,
    };
    assert!(valid_outcome(&outcome));
    outcome.status = ForcedExitStatus::Rejected;
    assert!(!valid_outcome(&outcome));
    for reason in 1..=5 {
    ...
    outcome.reason = 6;
    assert!(!valid_outcome(&outcome));
    outcome.status = ForcedExitStatus::Exited;
    outcome.reason = 0;
    outcome.recipient = auth().recipient;
    outcome.amount = 5;
    outcome.withdrawalHash = B256::repeat_byte(1);
    assert!(valid_outcome(&outcome));
    outcome.amount = 0;
    assert!(!valid_outcome(&outcome));
    assert_ne!(outcomes_hash(&[]), keccak256([]));
    assert_eq!(
        outcomes_hash(&[]),
...
fn solidity_interop_vectors() {
    ...
    let outcomes = [ForcedExitOutcome {
        requestId: 1,
        depositNumber: 2,
        token: TOKEN,
        recipient: a.recipient,
        status: ForcedExitStatus::Exited,
        reason: 0,
        amount: 123456,
        withdrawalHash: withdrawal_hash(&withdrawal),
    }];
    let values = serde_json::json!({
        ...
        "outcomesHash": outcomes_hash(&outcomes),
        "emptyOutcomesHash": outcomes_hash(&[]),
    });
