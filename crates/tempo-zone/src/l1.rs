...
impl EnabledToken {
    /// Encode a list of enabled tokens for embedding in a block header's `extra_data`.
    ///
    /// Format: `"ZTOK" || count:u16 || (address:20 || name_len:u8 || name || sym_len:u8 || sym || cur_len:u8 || cur)*`
    pub fn encode_extra_data(tokens: &[Self]) -> alloy_primitives::Bytes {
        if tokens.is_empty() {
            return alloy_primitives::Bytes::default();
        }
        let mut buf = Vec::new();
        buf.extend_from_slice(b"ZTOK");
        buf.extend_from_slice(&(tokens.len() as u16).to_be_bytes());
        for t in tokens {
            buf.extend_from_slice(t.token.as_slice());
            buf.push(t.name.len() as u8);
            buf.extend_from_slice(t.name.as_bytes());
            buf.push(t.symbol.len() as u8);
            buf.extend_from_slice(t.symbol.as_bytes());
            buf.push(t.currency.len() as u8);
            buf.extend_from_slice(t.currency.as_bytes());
        }
        alloy_primitives::Bytes::from(buf)
    }

    /// Decode a list of enabled tokens from a block header's `extra_data`.
    ///
    /// Returns an empty vec if the data is empty or does not start with `"ZTOK"`.
    pub fn decode_extra_data(data: &[u8]) -> Vec<Self> {
        if data.len() < 6 || &data[..4] != b"ZTOK" {
            return Vec::new();
        }
        ...
        for _ in 0..count {
            ...
            let token = Address::from_slice(&data[pos..pos + 20]);
            pos += 20;
            ...
            let sym_len = data[pos] as usize;
            pos += 1;
            let symbol = String::from_utf8_lossy(&data[pos..pos + sym_len]).into_owned();
            pos += sym_len;

            let cur_len = data[pos] as usize;
            pos += 1;
            let currency = String::from_utf8_lossy(&data[pos..pos + cur_len]).into_owned();
            pos += cur_len;
