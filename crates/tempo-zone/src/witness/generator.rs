...
impl WitnessGenerator {
    ...
    /// Generate a [`ZoneStateWitness`] from RPC `eth_getProof` responses.
    ///
    /// This is the async counterpart of [`generate_zone_state_witness`] — instead
    /// of using a reth `StateProvider`, it issues `eth_getProof` and `eth_getCode`
    /// calls against the Zone L2 RPC at the given block number (S₀).
    ///
    /// Used by the zone monitor to generate proofs after all blocks in the batch
    /// are known, allowing the access sets from all blocks to be merged first.
    pub async fn generate_zone_state_witness_from_rpc(
        &self,
        zone_rpc: &alloy_provider::DynProvider<tempo_alloy::TempoNetwork>,
        ...
        // Ensure predeploy accounts/slots are always present (same as the
        // state-provider path).
        accessed_accounts.insert(TEMPO_STATE_ADDRESS);
        accessed_storage
            .entry(TEMPO_STATE_ADDRESS)
        ...
        for &addr in &accessed_accounts {
            let storage_keys: Vec<alloy_primitives::StorageKey> = accessed_storage
                .get(&addr)
                .map(|slots| {
                    slots
                        .iter()
                        .map(|s| alloy_primitives::StorageKey::from(B256::from(s.to_be_bytes())))
                        .collect()
                })
                .unwrap_or_default();

            let proof_response = zone_rpc
                .get_proof(addr, storage_keys)
                .block_id(block_id)
                .await
            ...
            // Storage proofs → (slot, proof_nodes) and (slot, value).
            let mut storage_proofs_map: HashMap<U256, Vec<Bytes>> = HashMap::default();
            let mut storage_values: HashMap<U256, U256> = HashMap::default();

            for sp in &proof_response.storage_proof {
                let slot = U256::from_be_bytes(sp.key.as_b256().0);
                storage_proofs_map.insert(slot, sp.proof.clone());
                storage_values.insert(slot, sp.value);
            }

            // Fetch code if the account has it.
            let code = if proof_response.code_hash != keccak_empty
                && proof_response.code_hash != B256::ZERO
            {
                let code_bytes = zone_rpc
                    .get_code_at(addr)
                    .block_id(block_id)
                    .await
                    .map_err(|e| eyre::eyre!("eth_getCode failed for {addr}: {e}"))?;
            ...
            accounts.insert(
                ...
                AccountWitness {
                    ...
                    storage_proofs: storage_proofs_map,
                },
            );
        }
        ...
        debug!(
            ...
            absent = absent_accounts.len(),
            %state_root,
            "Generated zone state witness from RPC proofs"
        );
