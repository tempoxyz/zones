...
use zone_chainspec::ZoneHardforks as _;
...
/// execution. Before Z1 the replay may end at an open Zone tip without withdrawal
/// finalization. Every Z1 batch must end at a full block's finalization boundary.
pub fn prove_zone_batch(config: &SpfConfig, witness: BatchWitness) -> Result<BatchOutput, Error> {
    ...
    let first_z1 = witness.zone_blocks.iter().position(|block| {
        config
            .chain_spec()
            .zone_hardfork_at(block.timestamp)
            .is_z1()
    });
    if let Some(first_z1) = first_z1 {
        let z1_blocks = &witness.zone_blocks[first_z1..];
        let full_positions: Vec<_> = z1_blocks
            .iter()
            .enumerate()
            .filter_map(|(index, block)| block.tempo_headers_rlp.is_empty().then_some(index))
            .collect();
        if full_positions.len() != 1
            || full_positions[0] + 1 != z1_blocks.len()
            || z1_blocks
                .last()
                .is_some_and(|block| block.finalize_withdrawal_batch_count.is_none())
        {
            return Err(Error::InvalidZ1BatchShape);
        }
    }
    ...
    .to::<u64>();
    // Keep the public transition anchored to the physical pre-state. On the first Z1 batch this
...
pub enum Error {
    ...
    /// A Z1 batch must contain exactly one full operational block at the end, optionally preceded
    /// by checkpoint-only blocks.
    #[error("invalid Z1 batch shape")]
    InvalidZ1BatchShape,
...
mod tests {
    ...
    fn z1_test_config(activation: u64) -> SpfConfig {
        let tempo_chain_spec = tempo_chainspec::spec::MODERATO.clone();
        let mut genesis = tempo_chain_spec.genesis().clone();
        genesis.config.chain_id = zone_chain_id(tempo_chain_spec.chain().id(), 1).unwrap();
        genesis
    ...
    #[test]
    fn z1_batch_shape_rejects_multiple_full_blocks() {
        ...
        assert_eq!(
            prove_zone_batch(&z1_test_config(100), witness),
            Err(Error::InvalidZ1BatchShape)
        );
    }

    #[test]
    fn z1_batch_shape_rejects_checkpoint_only_batch() {
        let mut witness = minimal_batch_witness();
        witness.zone_blocks.push(ZoneBlock {
            number: 1,
            parent_hash: B256::ZERO,
