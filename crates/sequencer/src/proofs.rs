...
use zone_spf::{TempoStateWitness, TempoWitnessDatabase, WitnessDatabase, ZoneStateWitness};
...
/// State commitments inherited by the block whose proof sidecar is being validated.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct ProofParentState {
    pub zone_state_root: B256,
    pub tempo_number: u64,
    pub tempo_hash: B256,
}
...
/// Read the JSON sidecar for a block, if this node still retains it.
pub fn read_proof_json(directory: &Path, number: u64, hash: B256) -> Result<Option<Vec<u8>>> {
    let file_name = format!("{number}-{hash:x}.json");
    match fs::read(directory.join(file_name)) {
        Ok(json) => {
            validate_proof_json(&json, number, hash, None)?;
            Ok(Some(json))
        }
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(None),
        Err(error) => Err(error).wrap_err("read stored proof JSON"),
    }
}

/// Remove files covered by a portal-confirmed Zone checkpoint without starting a collector.
pub fn prune_proof_directory_through(directory: &Path, through: u64) -> Result<()> {
    drop(ProofStore::open(directory.to_path_buf(), through)?);
    Ok(())
}

/// Validate and atomically persist a proof sidecar received with a peer block.
pub fn persist_received_proof_json(
    directory: &Path,
    json: &[u8],
    number: u64,
    hash: B256,
    parent_hash: B256,
    parent_state: ProofParentState,
) -> Result<()> {
    let proof = validate_proof_json(json, number, hash, Some(parent_hash))?;
    WitnessDatabase::from_zone_state_witness(
        proof.zone_state_witness.clone(),
        parent_state.zone_state_root,
    )
    .context("proof sidecar does not authenticate the parent Zone state root")?;
    TempoWitnessDatabase::from_tempo_state_witness(proof.tempo_state_witness.clone())
        .context("proof sidecar does not authenticate its Tempo state root")?;
    let mut encoded_header = proof.tempo_state_witness.initial_tempo_header_rlp.as_ref();
    let tempo_header = TempoHeader::decode(&mut encoded_header)
        .context("decode proof sidecar initial Tempo header")?;
    ensure!(
        encoded_header.is_empty(),
        "proof sidecar initial Tempo header has trailing bytes"
    );
    ensure!(
        tempo_header.number() == parent_state.tempo_number
            && tempo_header.hash_slow() == parent_state.tempo_hash,
        "proof sidecar initial Tempo checkpoint does not match the parent Zone state"
    );
    fs::create_dir_all(directory)
        .wrap_err_with(|| format!("create proof directory {}", directory.display()))?;
    let path = directory.join(proof.file_name());
    let height_prefix = format!("{number}-");
    for entry in fs::read_dir(directory)
        .wrap_err_with(|| format!("read proof directory {}", directory.display()))?
    {
        let entry = entry?;
        let file_name = entry.file_name();
        if file_name.to_string_lossy().starts_with(&height_prefix) && entry.path() != path {
            bail!("conflicting proof already stored at Zone block {number}");
        }
    }
    if path.exists() {
        let existing =
            fs::read(&path).wrap_err_with(|| format!("read existing proof {}", path.display()))?;
        ensure!(existing == json, "received proof differs from stored proof");
        return Ok(());
    }
    let mut temporary = tempfile::NamedTempFile::new_in(directory)
        .wrap_err_with(|| format!("create temporary proof in {}", directory.display()))?;
    temporary
        .write_all(json)
        .context("write received block proof JSON")?;
    temporary
        .as_file()
        .sync_all()
        .context("sync received block proof JSON")?;
    fs::rename(temporary.path(), &path)
        .wrap_err_with(|| format!("publish received proof {}", path.display()))?;
    sync_directory(directory)
}

fn validate_proof_json(
    json: &[u8],
    number: u64,
    hash: B256,
    parent_hash: Option<B256>,
) -> Result<StoredBlockProof> {
    let proof: StoredBlockProof =
        serde_json::from_slice(json).context("decode block proof JSON")?;
    proof.validate()?;
    ensure!(proof.block_number == number, "proof block number mismatch");
    ensure!(proof.block_hash == hash, "proof block hash mismatch");
    if let Some(parent_hash) = parent_hash {
        ensure!(
            proof.parent_hash == parent_hash,
            "proof parent hash mismatch"
        );
    }
    Ok(proof)
}
...
impl<P: ZoneSequencerProvider> ProofCollector<P> {
    ...
    async fn reconcile_and_collect(&self) -> Result<()> {
        ...
        for (number, proof) in stored {
            ...
            if (!staged_next && number > head)
                || (number <= head && canonical != Some(proof.block_hash))
            {
