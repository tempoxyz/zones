...
#[tokio::test]
async fn test_sync_finalized_once_ingests_missing_finalized_range() {
    let subscriber = test_subscriber(9);
    ...
    subscriber
        .sync_finalized_once(&l1_provider, &mut next_block)
        .await
        .unwrap();
    subscriber
        .sync_finalized_once(&l1_provider, &mut next_block)
...
#[tokio::test]
async fn test_sync_finalized_once_preserves_progress_after_partial_failure() {
    let subscriber = test_subscriber(9);
    let asserter = Asserter::new();
    let l1_provider =
        ProviderBuilder::new_with_network::<TempoNetwork>().connect_mocked_client(asserter.clone());
