    ...
    P: StateProviderFactory + Sync,
{
    ...
    pub(crate) fn resolve_start_block(&self) -> Result<u64, L1SubscriberError> {
        ...
        // Only durable Zone state proves consumption; queued and observed blocks do not.
        self.block_tracker
            .initialize_consumed_through(local_checkpoint.number);
    ...
    pub(crate) async fn sync_finalized(
        ...
        let from = *next_block;
        let mut blocks = stream::iter(from..=finalized)
        ...
        while let Some((header, receipts)) = blocks.try_next().await? {
            ...
            self.apply_block(header, &receipts).await?;
    ...
    /// Applies a receipt-verified block before advancing the sync cursor.
            ...
            for submission in finalized_batches {
                sender
                    ...
                    .map_err(|_| L1SubscriberError::Fatal {
                        ...
                        source: eyre::eyre!("finalized batch submission observer is unavailable"),
                    })?;
            }
        }
    ...
    pub async fn run(self) -> Result<(), L1SubscriberError> {
        ...
        loop {
            let result: Result<(), L1SubscriberError> = async {
                let next_block = match &mut next_block {
                    Some(next_block) => next_block,
                    cursor @ None => cursor.insert(self.resolve_start_block()?),
                };
                ...
                self.sync_finalized(&provider, next_block).await?;
