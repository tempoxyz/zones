...
mod tests {
    ...
    fn counter_value(snapshot: &SnapshotEntries, name: &str) -> u64 {
        snapshot
            .iter()
    ...
    fn counter_delta(before: &SnapshotEntries, after: &SnapshotEntries, name: &str) -> u64 {
        counter_value(after, name).saturating_sub(counter_value(before, name))
    }

    fn with_metrics_snapshot<T>(
        action: impl FnOnce() -> T,
    ) -> (T, SnapshotEntries, SnapshotEntries) {
    ...
    fn refresh_success_increments_attempt_metric() {
        let ((_, expires_at), before, after) = with_metrics_snapshot(|| {
            let provider = ZoneProvider::new(test_config()).unwrap();
            let mut state = provider.state.lock();
            state.expires_at = 0;
            let refreshed = provider.refresh_provider_with(&mut state, build_provider_with_token);
            (refreshed, state.expires_at)
        });
    ...
    fn refresh_failure_reuses_stale_provider() {
        let ((before_provider, after_provider, expires_at), before, after) =
            with_metrics_snapshot(|| {
                let provider = ZoneProvider::new(test_config()).unwrap();
                let mut state = provider.state.lock();
                state.expires_at = 0;
                let before_provider = state.provider.clone();
                let after_provider = provider.refresh_provider_with(&mut state, |_| {
                    Err(eyre::eyre!("forced refresh failure"))
                });
                (before_provider, after_provider, state.expires_at)
            });

        assert!(std::ptr::eq(before_provider.root(), after_provider.root()));
