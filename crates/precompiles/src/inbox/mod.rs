...
impl ZoneInbox {
    ...
    fn fail_deposit(
        ...
        // NOTE: jtcn 169: A deposit bounces when its proof or ciphertext is bad, or its Zone
    ...
    fn process_withdrawal_bounce_back(
        ...
        // NOTE: jtcn 179: Resolves and deletes the saved fallback nonce, then mints the amount to
    ...
    fn claim_refund(&mut self, caller: Address, token: Address) -> ZoneResult<u128> {
        // NOTE: jtcn 180: If policy blocks the bounceback mint, the Inbox parks it under the token
    ...
    // NOTE: jtcn 181: Checkpoint: A failed Zone withdrawal used the portal deposit queue and
