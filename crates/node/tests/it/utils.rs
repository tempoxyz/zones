...
impl L1TestNode {
    ...
    /// Start in T12 with a genesis portal whose mock verifier survives the T13 runtime upgrade.
    pub(crate) async fn start_with_t13_portal(activation: u64) -> eyre::Result<(Self, Address)> {
        ...
        install_native_zone_factory(&mut genesis, signer.address())?;
        genesis
            .config
            .extra_fields
            .insert_value("t13Time".into(), activation)?;
        ...
        let node = Self::start_with(|cfg| {
            cfg.chain = Arc::new(TempoChainSpec::from_genesis(genesis));
            cfg.dev.block_time = None;
        })
        .await?;
        node.fund_user(admin, 10_000_000).await?;
        let encryption_key = k256::SecretKey::from(signer.credential());
        node.set_sequencer_encryption_key(portal_address, &encryption_key)
            .await?;
        Ok((node, portal_address))
    }
