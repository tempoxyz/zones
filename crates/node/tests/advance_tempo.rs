...
use tempo_precompiles::zone_factory::zone_portal_slots::{
    ADMIN, CURRENT_DEPOSIT_QUEUE_HASH, ENCRYPTION_KEYS, IS_SEQUENCER,
};
...
fn zone_portal_storage_slot_constants_match_solidity() {
    assert_eq!(Into::<B256>::into(ADMIN), B256::ZERO, "admin is slot 0");
    assert_eq!(
        Into::<B256>::into(CURRENT_DEPOSIT_QUEUE_HASH),
        B256::from(U256::from(3)),
        "currentDepositQueueHash is slot 3"
    );
    assert_eq!(
        Into::<B256>::into(ENCRYPTION_KEYS),
        B256::from(U256::from(5)),
        "_encryptionKeys is slot 5"
    );
    assert_eq!(
        Into::<B256>::into(IS_SEQUENCER),
