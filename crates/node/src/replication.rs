    ...
    P: PersistedBlockSource,
{
    for number in last_broadcast.saturating_add(1)..=tip_number {
        ...
        // NOTE: jtcn 157: Reads each newly saved Zone block from the DB and sends it to the P2P
    ...
    P: HeaderProvider<Header = TempoHeader> + ReceiptProvider,
{
    ...
    // NOTE: jtcn 172: Verifies each follower signature and saves it under the exact batch hash.
    ...
    loop {
        ...
        biased;
            ...
            response = backfill_responses.recv() => {
                // NOTE: jtcn 164: Missing blocks returned by a peer enter through the follower's
                ...
                match event {
                    ...
                    P2pEvent::SettlementProposalReceived { leader, proposal } => {
                        let result = async {
                            ...
                            // NOTE: jtcn 170: The follower rebuilds the proposed batch from its own
                            ...
                            // NOTE: jtcn 171: Sends the follower's signature back to the leader over
                    ...
                    P2pEvent::BlockReceived { .. } => {
                        // NOTE: jtcn 160: A live block enters here after Commonware receives it and
            ...
            _ = retry.tick(), if backfill.needed => {
                ...
                // NOTE: jtcn 162: Sends the first missing height to the backfill coordinator. The
        ...
        + 'static,
{
    ...
    if number > best.saturating_add(1) {
        // NOTE: jtcn 161: If a block skips a height, it stays in `pending`. This marks backfill as
        ...
        + 'static,
{
    loop {
        ...
        // NOTE: jtcn 165: Before import, checks the height, parent, scheduled leader, L1 checkpoint,
        ...
        + 'static,
{
    ...
    // NOTE: jtcn 166: Checkpoint: The follower received a live or backfilled block, waited for its
