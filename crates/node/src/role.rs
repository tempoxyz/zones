    ...
    Pool: TransactionPool<Transaction = TempoPooledTransaction> + Clone + 'static,
{
    ...
    loop {
        // NOTE: jtcn 115: Reads who owns the next L1 block and runs this node as leader, follower,
        ...
        if !current
            ...
            .is_some_and(|generation| generation.role.same_variant(desired))
        {
            // NOTE: jtcn 116: Stops every task from the old role before starting the new one. This
    ...
    Pool: TransactionPool<Transaction = TempoPooledTransaction> + Clone + 'static,
{
    ...
    match desired {
        DesiredRole::Fenced => {
            // NOTE: jtcn 118: Fenced means the node cannot prove who should lead next. It stops
        ...
        DesiredRole::Follower { epoch } => {
            // NOTE: jtcn 98: Creates the follower channels. Live P2P events go to `sync_rx`,
            ...
            tasks.spawn(async move {
                tokio::select! {
                    ...
                    // NOTE: jtcn 106: Sends local transactions to the current leader and the next
            ...
            // NOTE: jtcn 112: Checkpoint: P2P carries saved blocks, missing block recovery,
        ...
        DesiredRole::Leader { epoch, .. } => {
            ...
            let commands = context.commands.clone();
            tasks.spawn(async move {
                // NOTE: jtcn 95: Starts the block broadcaster. It only publishes Zone blocks that
    ...
    // NOTE: jtcn 120: Checkpoint: A finalized portal event changed the schedule. The old role
