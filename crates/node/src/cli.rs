...
impl ZoneCli {
    fn command() -> clap::Command {
        Cli::<ZoneChainSpecParser, ZoneArgs>::command()
            ...
            .mut_subcommand("node", |command| {
                command
                    .mut_arg("dev", |arg| {
                        arg.help("Provision a fresh zone against a Tempo dev L1 and run it")
                            .long_help(None)
                    })
                    .mut_arg("block_max_transactions", |arg| arg.hide(true))
                    .mut_arg("block_time", |arg| arg.hide(true))
                    .mut_arg("finality_depth", |arg| arg.hide(true))
                    .mut_arg("payload_wait_time", |arg| arg.hide(true))
                    .mut_arg("dev_mnemonic", |arg| arg.hide(true))
            })
    }
        ...
        T: Into<std::ffi::OsString> + Clone,
    {
        ...
        if let Some(args) = dev_node_args(&args) {
            return DevCommand::command()
                .bin_name("tempo-zone node --dev")
                .try_get_matches_from(args)
                .and_then(|matches| DevCommand::from_arg_matches(&matches))
                .map(Box::new)
                .map(Self::Dev);
        }
...
/// Routes `tempo-zone node --dev` to the Zone dev harness before Reth can enable its local miner.
fn dev_node_args(args: &[std::ffi::OsString]) -> Option<Vec<std::ffi::OsString>> {
    if args.get(1)?.to_str()? != "node" {
        return None;
    }
    ...
    let dev_index = args
        ...
        .enumerate()
        .skip(2)
        .take_while(|(_, arg)| arg.to_str() != Some("--"))
        .find_map(|(index, arg)| (arg.to_str() == Some("--dev")).then_some(index))?;

    Some(
        args.iter()
            .enumerate()
            .filter(|(index, _)| *index != 1 && *index != dev_index)
            .map(|(_, arg)| arg.clone())
            .collect(),
    )
}
...
pub(crate) fn run_dev_node(mut cli: Cli<ZoneChainSpecParser, ZoneArgs>) -> eyre::Result<()> {
    ...
    cli.run_with_components::<ZoneNode>(components, async move |mut builder, args| {
        ...
        eyre::ensure!(
            ...
            "--sequencer.manifest is not supported with `tempo-zone node --dev`"
        );
...
pub struct ZoneArgs {
    ...
    /// development should use `tempo-zone node --dev` instead.
    #[arg(
        long = "sequencer",
        env = "SEQUENCER",
...
mod tests {
    ...
    use super::{
        Role, ZoneArgs, ZoneCli, dev_node_args, load_decryption_keys, load_sequencer_signer,
        parse_l1_rpc_url, parse_portal_address, validate_deprecated_zone_id,
        validate_p2p_transaction_size_limit,
    };
    ...
    #[test]
    fn node_help_describes_zone_dev_behavior() {
        let error = ZoneCli::try_parse_from(["tempo-zone", "node", "--help"])
            .err()
            .expect("--help exits through clap");
        let help = error.to_string();
        assert!(help.contains("Provision a fresh zone against a Tempo dev L1 and run it"));
        assert!(!help.contains("--dev.block-time"));
    }
    ...
    #[test]
    fn node_dev_is_routed_to_the_zone_dev_harness() {
        let parsed = ZoneCli::try_parse_from(["tempo-zone", "node", "--dev"]).unwrap();
        assert!(matches!(parsed, ZoneCli::Dev(_)));
    }

    #[test]
    fn forwarded_dev_flag_is_not_treated_as_the_zone_dev_command() {
        let args = ["tempo-zone", "node", "--", "--dev"].map(std::ffi::OsString::from);
        assert!(dev_node_args(&args).is_none());
    }
