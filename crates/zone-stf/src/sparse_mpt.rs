...
impl SparseTrie {
    ...
    pub fn from_proofs(proofs: &[(Nibbles, &[Bytes])]) -> Result<Self, ProverError> {
        ...
        let branch_keys: Vec<Nibbles> = entries
            .iter()
            .filter(|&(_k, v)| matches!(v, TrieEntry::Branch(_)))
            .map(|(k, _v)| *k)
            .collect();
...
mod tests {
    ...
    fn build_storage_trie_with_proofs(
        ...
        let targets: Vec<Nibbles> = sorted.iter().map(|(k, _, _)| *k).collect();
    ...
    fn build_state_trie_with_absence_proof(
        ...
        let mut targets: Vec<Nibbles> = sorted.iter().map(|(k, _, _)| *k).collect();
        targets.push(absent_key);
    ...
    fn test_multiple_leaves_consistent_root() {
        ...
        for i in 0u64..5 {
            ...
            trie1.update_leaf(key, encoded.clone());
