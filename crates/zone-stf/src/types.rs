...
impl From<revm::database_interface::bal::EvmDatabaseError<Self>> for ProverError {
    fn from(error: revm::database_interface::bal::EvmDatabaseError<Self>) -> Self {
        match error {
            revm::database_interface::bal::EvmDatabaseError::Database(inner) => inner,
            revm::database_interface::bal::EvmDatabaseError::Bal(inner) => {
                Self::ExecutionError(format!("revm BAL database error: {inner}"))
            }
        }
    }
}
