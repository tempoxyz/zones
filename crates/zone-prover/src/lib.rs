...
{
    let value = db.storage(address, slot).map_err(Into::into)?;
    Ok(B256::from(value.to_be_bytes()))
}

/// Read ZoneOutbox.lastBatch from the zone state.
fn read_last_batch_from_db<DB>(db: &mut DB) -> Result<LastBatch, ProverError>
where
    DB: Database,
    DB::Error: Into<ProverError>,
{
...
/// previous batch.
fn compute_tempo_block_hash<DB>(witness: &BatchWitness, db: &mut DB) -> Result<B256, ProverError>
where
    DB: Database,
    DB::Error: Into<ProverError>,
{
