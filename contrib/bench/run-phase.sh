...
if (( 10#$ZONES_BENCH_DRAIN_TIMEOUT > 0 )); then
    txpool_response="$({
        curl --silent --show-error --fail \
            ...
            "$submit_rpc"
    } 2>&1)" || die "selected RPC did not answer txpool_status"
    if ! jq -e '.error == null and (.result | type == "object")' \
        >/dev/null <<<"$txpool_response"; then
        die "selected RPC does not expose txpool_status; expose txpool internally or set ZONES_BENCH_DRAIN_TIMEOUT=0"
    fi
fi
