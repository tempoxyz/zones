# Prevent crossed orders and allow same-tick flip orders

This document specifies changes to the Stablecoin DEX that (1) prevent placing orders that would cross existing orders on the opposite side of the book, and (2) allow flip orders to flip to the same tick.
