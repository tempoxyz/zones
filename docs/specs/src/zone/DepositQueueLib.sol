...
/// @title DepositQueue
/// @notice 3-slot queue for L1→zone deposits with proof target pattern
struct DepositQueue {
...
///      - `snapshot`: snapshot of `current` at proof start (stable proof target)
///      - `current`: head where new deposits land
library DepositQueueLib {
    error UnexpectedSnapshotHash();
    ...
    function dequeueWithProof(
        ...
        if (queue.snapshot != transition.prevSnapshotHash) revert UnexpectedSnapshotHash();
    ...
    /// @return snapshot Stable proof target for the current proof
    ...
    function getState(
        ...
        return (queue.processed, queue.snapshot, queue.current);
    }
}
