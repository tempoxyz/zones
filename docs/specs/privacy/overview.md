...
The portal accepts the batch only if the verifier confirms that the state transition is valid. Once the batch is accepted, it commits the ZonePortal on Tempo to:
...
To move assets from the zone to Tempo, a user requests a withdrawal inside `ZoneOutbox`.
...
- `balanceOf` is restricted so users can only see balances of accounts they own.
...
Privacy is also enforced at the RPC layer:
...
|----------|----------------------|-----------------------------|
| Correctness | Assuming the verifier is sound, the sequencer cannot forge state transitions or steal locked funds by posting an invalid batch. | A critical bug in the verifier means that a malicious sequencer could steal funds. |
| Withdrawals | Once a token is enabled, users keep a withdrawal path for that token. Failed callback withdrawals bounce back instead of blocking the queue. | Zones do not guarantee exits. The sequencer still has to keep processing batches and withdrawals. |
...
- The sequencer can transfer control through a two-step handoff.
