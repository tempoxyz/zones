...
contract ZoneIntegrationTest is BaseTest {
    ...
    function test_multiUserDeposit_processedCorrectly() public {
        ...
        vm.startPrank(alice);
        pathUSD.approve(address(l1Portal), 10000e6);
    ...
    function test_multipleBatches_withdrawalsInDifferentSlots() public {
        // Initial deposit
        vm.startPrank(alice);
        pathUSD.approve(address(l1Portal), 100000e6);
        bytes32 depositHash = l1Portal.deposit(alice, 50000e6, bytes32("big deposit"));
        vm.stopPrank();

        // Process on L2
        Deposit[] memory deposits = new Deposit[](1);
        deposits[0] = Deposit({ sender: alice, to: alice, amount: 50000e6, memo: bytes32("big deposit") });
        l2TempoState.setMockStorageValue(address(l1Portal), CURRENT_DEPOSIT_QUEUE_HASH_SLOT, depositHash);
        vm.prank(admin);
        l2Inbox.advanceTempo("", deposits);
    ...
    function test_mixedFlow_depositsAndWithdrawalsInterleaved() public {
        ...
        vm.startPrank(alice);
        pathUSD.approve(address(l1Portal), 100000e6);
        l1Portal.deposit(alice, 10000e6, bytes32("d1"));
        vm.stopPrank();
        ...
        deposits1[0] = Deposit({ sender: alice, to: alice, amount: 10000e6, memo: bytes32("d1") });
        ...
        vm.startPrank(charlie);
        pathUSD.approve(address(l1Portal), 20000e6);
        ...
        assertEq(l2GasToken.balanceOf(alice), 10000e6 - 2000e6);
    ...
    function test_invariant_totalSupplyMatchesNetDeposits() public {
        ...
        vm.startPrank(alice);
        pathUSD.approve(address(l1Portal), 10000e6);
        bytes32 d1 = l1Portal.deposit(alice, 10000e6, bytes32("d1"));
        vm.stopPrank();

        Deposit[] memory deposits = new Deposit[](1);
        deposits[0] = Deposit({ sender: alice, to: alice, amount: 10000e6, memo: bytes32("d1") });
        l2TempoState.setMockStorageValue(address(l1Portal), CURRENT_DEPOSIT_QUEUE_HASH_SLOT, d1);
        vm.prank(admin);
        l2Inbox.advanceTempo("", deposits);

        assertEq(l2GasToken.totalSupply(), 10000e6);
