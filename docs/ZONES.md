...
| `--sequencer.prover-address` | (optional) | Repeatable `HARDFORK=HOST:PORT` assignment selected by the live L1 fork; missing forks stop proving |
| `--sequencer.prover-attestation-policy` | (required with remote addresses) | JSON PCR0–2 allowlist and evidence-freshness policy shared by the hardfork endpoints for Nitro-attested TLS |
...
| `SEQUENCER_PROVER_ATTESTATION_POLICY` | With remote provers | JSON PCR0–2 allowlist and evidence-freshness policy path shared by the configured endpoints |
