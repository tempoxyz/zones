...
The durable onchain checkpoint is `tempoBlockHash`, `tempoBlockNumber`, and `tempoStateRoot`. Before the first Tempo import all three are zero. Once initialized, `tempoBlockHash` is always `keccak256(RLP(TempoHeader))` and `tempoStateRoot` is the state root decoded from that header.
...
`ZoneInbox.advanceTempo()` calls `TempoState.finalizeTempo(header)` to advance the zone's view of Tempo. This function decodes the RLP header, validates chain continuity (parent hash must match the previous finalized header, block number must increment by one), stores the new block hash, block number, and state root, and emits the checkpoint in `TempoBlockFinalized`.
...
5. **Execute `advanceTempo` as the first transaction.**
   Call `TempoState.finalizeTempo(header)` using the required `tempo_header_rlp` in the modeled execution environment. If the stored `tempoBlockHash` is non-zero, require the imported header's number to be the stored `tempoBlockNumber + 1` and its parent hash to equal the stored `tempoBlockHash`. If the stored hash is zero, instead verify a Tempo state proof for this portal's `sequencer` slot against the imported header's state root and require the value to be non-zero. Then update the bound `tempoBlockNumber`, `tempoBlockHash`, and `tempoStateRoot`. Subsequent Tempo storage reads use that persisted checkpoint. Require the finalized `tempoBlockHash` to equal `keccak256(tempo_header_rlp)` and `tempoStateRoot` to equal the imported header's state root. Reject any ordinary block that omits or executes more than one `advanceTempo` call.
...
10. **Extract the final batch commitments from the post-state.**
    Read the final `ZoneInbox.processedDepositQueueHash`, `ZoneOutbox.lastBatch`, `TempoState.tempoBlockNumber`, `TempoState.tempoBlockHash`, and `TempoState.tempoStateRoot` from the executed state.

11. **Verify the batch's final Tempo binding and anchor.**
    Require `TempoState.tempoBlockNumber == public_inputs.tempo_block_number` and `TempoState.tempoStateRoot` to equal the state root of the finalized Tempo header. If `anchor_block_number == tempo_block_number`, require `TempoState.tempoBlockHash == anchor_block_hash`. Otherwise, verify the parent-hash chain from `tempo_block_number` to `anchor_block_number` using `tempo_ancestry_headers`, ending at `anchor_block_hash`. A bootstrap genesis proof that has not imported Tempo instead requires the three stored Tempo checkpoint fields and all four Tempo binding and anchor inputs to be zero.
...
For the bootstrap transition from zero to the canonical zone genesis block, `tempoBlockNumber` and `recentTempoBlockNumber` are both zero and the portal skips the EIP-2935 lookup. The genesis proof must show that `TempoState.tempoBlockHash`, `tempoBlockNumber`, and `tempoStateRoot` remain zero.
