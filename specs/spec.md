...
Zone-owned hardforks have an independent timestamp schedule in the chain configuration. `z2Time` activates Z2's mandatory same-block deposit finalization in both node execution and STF replay; if omitted, Z2 is never activated. This change sets no activation timestamp for existing chains. Operators must coordinate node and prover upgrades and a future activation boundary before enabling the rule.

Tempo hard fork upgrades activate on Zones in lockstep with Tempo. A zone block's timestamp selects its execution rules. The node MUST NOT produce a block under new Tempo rules until the finalized Tempo chain has activated the same fork, even when the block imports an older Tempo checkpoint during catch-up.
