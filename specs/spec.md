...
**Tempo-side refund.** The bounce-back withdrawal is submitted in the next batch alongside any user-initiated withdrawals. When `ZonePortal.processWithdrawals` runs on the deposit-bounce-back entry (`gasLimit == 0`, `fallbackNonce == 0`), it computes `bouncebackFee = min(ceil(bouncebackGas * block.basefee / 1e12), amount)`, attempts to pay it to the portal admin, and attempts to deliver `amount - bouncebackFee` from the portal's escrow. Before each transfer, the portal validates the recipient's TIP-1028 receive policy using the portal as the transfer sender; a blocked policy is treated as failed delivery without invoking TIP-20, so funds cannot be redirected to `ReceivePolicyGuard`. Token calls remain wrapped in `try/catch`.
...
For a plain withdrawal (`gasLimit == 0`), the portal rechecks the current modes and roles and prechecks the recipient's TIP-1028 receive policy using the portal as the transfer sender. If either check blocks direct delivery, the portal does not attempt the transfer and creates a withdrawal bounce-back deposit for the Zone-local `zoneFallbackRecipient`. A transfer that reverts or returns false creates the same bounce-back.
