...
library FeeMath {
        ...
        returns (FeePreview memory result, uint256[5] memory nextRemainders)
    {
        ...
        if (input.supply == 0 || input.shareScale == 0) return (result, remainders);

        result.preFeeValuePerShare = Math.mulDiv(input.activeAssets, input.shareScale, input.supply);
        result.targetValuePerShare = config.excess.enabled
            ? targetAt(
                input.targetBase,
        ...
        if (result.preFeeValuePerShare > input.highWaterMark) {
            result.positiveAccrualAssets = Math.mulDiv(
                result.preFeeValuePerShare - input.highWaterMark, input.supply, input.shareScale
            );
        }
        ...
        if (config.excess.enabled) {
            uint256 targetAssets =
                Math.mulDiv(result.targetValuePerShare, input.supply, input.shareScale);
        ...
        if (result.totalFeeAssets != 0 && result.totalFeeAssets < input.activeAssets) {
            result.totalFeeShares = Math.mulDiv(
                result.totalFeeAssets, input.supply, input.activeAssets - result.totalFeeAssets
            );
            _allocateShares(result);
        }

        uint256 postSupply = input.supply + result.totalFeeShares;
        result.postFeeValuePerShare = Math.mulDiv(input.activeAssets, input.shareScale, postSupply);
    }
    ...
    function _allocateShares(FeePreview memory result) private pure {
        uint256 assigned = 0;
        ...
        for (uint256 i = 0; i < result.allocationCount; i++) {
            if (i == lastNonzero) {
                result.allocations[i].feeShares = result.totalFeeShares - assigned;
            } else if (result.allocations[i].feeAssets != 0) {
                uint256 shares = Math.mulDiv(
                    result.totalFeeShares, result.allocations[i].feeAssets, result.totalFeeAssets
                );
                result.allocations[i].feeShares = shares;
                assigned += shares;
            }
        }
    }
