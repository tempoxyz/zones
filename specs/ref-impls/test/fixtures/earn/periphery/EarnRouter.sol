...
contract EarnRouter is IWithdrawalReceiver {
        ...
        returns (uint256 earnShares, bytes32 zoneDepositHash)
    {
        ...
        emit PublicDepositRoutedToZone(
            msg.sender, earnVault, delivery.portal, assets, earnShares, zoneDepositHash
        );
    }
    ...
    function depositFromVault(
        ...
        external
        nonReentrant
        returns (uint256 assets, uint256 earnShares)
    {
        (address vaultAsset,) = _earnVaultTokens(earnVault);
        if (recipient == address(0)) revert ZeroAddress();
        uint256 balanceBefore = IERC20Like(vaultAsset).balanceOf(address(this));
        ...
        _requireBalance(vaultAsset, balanceBefore);
        emit VaultDepositRouted(
            msg.sender, earnVault, sourceVault, recipient, sourceVaultShares, assets, earnShares
        );
    }
    ...
    function depositFromVaultToZone(
        ...
        external
        nonReentrant
        returns (uint256 assets, uint256 earnShares, bytes32 zoneDepositHash)
    {
        (address vaultAsset, address earnShare) = _earnVaultTokens(earnVault);
        _validateZoneDelivery(delivery, earnShare);
        uint256 assetBalanceBefore = IERC20Like(vaultAsset).balanceOf(address(this));
        ...
        returns (uint256 assets, bytes32 zoneDepositHash)
    {
        ...
        _requireBalance(earnShare, earnShareBalanceBefore);
        _requireBalance(vaultAsset, assetBalanceBefore);
        emit PublicRedeemRoutedToZone(
            msg.sender, earnVault, delivery.portal, earnShares, assets, zoneDepositHash
        );
    }
        ...
        returns (uint256 assets, uint256 earnShares)
    {
        ...
        if (sourceVaultShares == 0 || minRedeemedAssets == 0 || minEarnShares == 0) {
            revert ZeroAmount();
        }
        ...
        internal
    {
        ...
        if (data.destination == Destination.Public) {
            ...
            if (flow == Flow.Deposit) {
                _handleDepositToPublic(
                    data.earnVault, vaultAsset, earnShare, token, amount, recipient, data
                );
            } else {
                _handleRedeemToPublic(
                    data.earnVault, vaultAsset, earnShare, token, amount, recipient, data
                );
            }
        ...
        internal
    {
        ...
        uint256 earnShares =
            _depositHeldAssets(earnVault, vaultAsset, vaultAssets, data.minEarnShares, recipient);
        emit PrivateDepositRoutedToPublic(
            data.actionId, earnVault, recipient, token, amount, vaultAssets, earnShares
        );
    }
        ...
        internal
    {
        uint256 earnShareBalanceBefore = IERC20Like(earnShare).balanceOf(address(this));
        uint256 vaultAssets = _prepareDeposit(earnVault, vaultAsset, earnShare, token, amount, data);
        uint256 earnShares = _depositHeldAssets(
            earnVault, vaultAsset, vaultAssets, data.minEarnShares, address(this)
        );
        bytes32 zoneDepositHash = _returnEncryptedToSourceZone(
            sourcePortal, earnShare, _toUint128(earnShares), destination
        );
        _requireBalance(earnShare, earnShareBalanceBefore);
        emit EarnDeposit(
            data.actionId, earnVault, token, amount, vaultAssets, earnShares, zoneDepositHash
        );
    }
        ...
        internal
    {
        ...
        (uint256 vaultAssets, uint256 outputAmount) =
            _prepareRedeem(earnVault, vaultAsset, earnShare, token, amount, data);
        bytes32 zoneDepositHash = _returnEncryptedToSourceZone(
            sourcePortal, data.outputToken, _toUint128(outputAmount), destination
        );
        ...
        returns (uint256 vaultAssets)
    {
        ...
        if (token == vaultAsset) {
            ...
            vaultAssets = _swap(
                IEarnVault(earnVault).depositSwapOverride(token),
                token,
                vaultAsset,
                amount,
                data.minVaultAssets
    ...
    function _prepareRedeem(
        ...
        internal
        returns (uint256 vaultAssets, uint256 outputAmount)
    {
        if (token != earnShare) revert WrongEarnShare();
        if (data.outputToken == address(0) || data.outputToken == earnShare) {
            revert WrongOutputToken();
        }

        vaultAssets =
            _redeemHeldEarnShares(earnVault, earnShare, amount, data.minVaultAssets, address(this));
        ...
        returns (uint256 amountOut)
    {
        ...
        if (overrideAdapter == address(0)) {
            ITempoStablecoinDex(STABLECOIN_DEX)
                .swapExactAmountIn(
                    tokenIn, tokenOut, _toUint128(amountIn), _toUint128(minAmountOut)
                );
        } else {
            ISwapAdapter(overrideAdapter)
                .swapExactIn(tokenIn, tokenOut, amountIn, address(this), minAmountOut);
        }
        ...
        view
    {
        ...
        if (
            zone.portal != sourcePortal
                || !IZoneFactory(ZONE_FACTORY_ADDRESS).isZonePortal(sourcePortal)
        ) {
    ...
    function _validateZoneDelivery(ZoneDelivery memory delivery, address token) internal view {
        if (delivery.portal == address(0) || delivery.refundRecipient == address(0)) {
            revert ZeroAddress();
        }
    ...
    function _validateTargetPortal(address portal, address token) internal view {
        ...
        if (
            !IZonePortal(portal).isTokenEnabled(token)
                || !IZonePortal(portal).areDepositsActive(token)
        ) {
            revert InvalidToken();
        }
    }

    function _depositEncryptedToZone(
        address token,
        uint128 amount,
        ZoneDelivery memory delivery
    )
        internal
        returns (bytes32 hash)
    {
        ...
        hash = IZonePortal(delivery.portal)
            .depositEncrypted(
                token, amount, delivery.keyIndex, delivery.encrypted, delivery.refundRecipient
            );
        ...
        returns (bytes32 hash)
    {
        ...
        hash = IZonePortal(sourcePortal)
            .depositEncrypted(
                token,
                amount,
                destination.keyIndex,
                destination.encrypted,
                destination.refundRecipient
            );
        _safeApprove(token, sourcePortal, 0);
    }

    function _earnVaultTokens(address earnVault)
        internal
        view
        returns (address vaultAsset, address earnShare)
    {
