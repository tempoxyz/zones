...
import {
    ...
    ITIP20IssuerAccess,
    ITIP20Metadata
} from "../interfaces/external/tempo/ITIP20Factory.sol";
...
contract EarnFactory {
        ...
        returns (address earnShare, EarnVault earnVault, EarnFees earnFees)
    {
        ...
        if (
            bytes(engineName).length == 0 || bytes(engineSymbol).length == 0
                || bytes(earnCurrency).length == 0
        ) {
        ...
        bytes32 earnShareSalt = _earnShareSalt(
            params.deploymentId, params.engine, params.owner, controlConfigHash, feeConfigHash
        );
        address predictedEarnShare =
            ITIP20Factory(tip20Factory).getTokenAddress(address(this), earnShareSalt);
        ...
        earnVault.initialize(
            params.engine, earnShare, address(earnFees), params.owner, params.controls, params.fees
        );
    ...
    function predictEarnFees(DeployParams calldata params) external view returns (address) {
        ...
        return Clones.predictDeterministicAddress(
            earnFeesImplementation, _earnFeesSalt(earnShareSalt), address(this)
        );
    }
    ...
    function _grantEarnVaultIssuerRole(address earnShare, address earnVault) internal {
        ...
        if (!ITIP20IssuerAccess(earnShare).hasRole(earnVault, issuerRole)) {
            revert IssuerGrantFailed();
        }
    }
    ...
    function _handoffShareAdmin(address earnShare, address owner) internal {
        ...
        if (!ITIP20IssuerAccess(earnShare).hasRole(owner, DEFAULT_ADMIN_ROLE)) {
