...
contract EarnFees is IEarnFees {
    ...
    function pendingFeeSnapshot(bytes32 requestId)
        external
        view
        returns (PendingFeeSnapshot memory)
    {
        ...
        returns (uint64 configId)
    {
        ...
        uint256 oldTargetNow = oldConfig.excess.enabled
            ? EarnFeeMath.targetAt(
                targetBase, oldConfig.excess.annualTargetRate, targetStartedAt, block.timestamp
            )
            : 0;
            ...
            if (!wasActive || reactivated) {
                ...
                if (oldConfig.excess.enabled) {
                    ...
                    targetBase = earnShareSupply == 0
                        ? 0
                        : Math.mulDiv(
                            IEarnEngine(_engine()).totalAssets(), earnShareScale, earnShareSupply
                        );
                    targetStartedAt = uint40(block.timestamp);
                }
            } else {
        ...
        onlyEarnVault
    {
        ...
        if (config.excess.enabled) {
            uint256 currentTarget = EarnFeeMath.targetAt(
        ...
        returns (uint256 feeEarnShares)
    {
        ...
        if (chargePending) {
            ...
            feeEarnShares = returnedValue == 0
                ? 0
                : Math.mulDiv(totalReentryEarnShares, pendingFees.totalFeeAssets, returnedValue);
    ...
    function claim(address to, uint256 earnShares) external nonReentrant {
        if (to == address(0) || to == address(this) || to == earnVault) {
            revert InvalidFeeClaimReceiver();
        }
    ...
    function _accrueFees() internal returns (FeePreview memory result) {
        ...
        if (result.preFeeValuePerEarnShare > highWaterMark) {
            highWaterMark = result.postFeeValuePerEarnShare;
        }

        FeeConfig memory config = _feeConfigs[currentFeeConfigId];
        if (
            config.excess.enabled
                && result.postFeeValuePerEarnShare > result.targetValuePerEarnShare
        ) {
    ...
    function _previewPendingFees(
        PendingFeeSnapshot storage pending,
        uint256 returnedValue
    )
        internal
        view
    ...
    function _creditCalculatedAllocations(uint64 configId, FeePreview memory fees) internal {
        for (uint256 i = 0; i < fees.allocationCount; i++) {
            ...
            if (earnShares != 0) {
                _creditFeeEarnShares(
                    configId, fees.allocations[i].account, fees.allocations[i].feeAssets, earnShares
                );
            }
        }
    }

    function _creditAllocatedFeeEarnShares(
        uint64 configId,
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < fees.allocationCount; i++) {
            ...
            if (i == last) {
                ...
                earnShares =
                    Math.mulDiv(feeEarnShares, fees.allocations[i].feeAssets, fees.totalFeeAssets);
                assigned += earnShares;
            }
            if (earnShares != 0) {
                _creditFeeEarnShares(
        ...
        internal
    {
        ...
        uint256 currentValue =
            Math.mulDiv(IEarnEngine(_engine()).totalAssets(), earnShareScale, normalization);
    ...
    function _waiveClosedHistoricalRemainders(uint64 configId) internal {
        if (
            configId == 0 || configId == currentFeeConfigId
                || _openRequestsByFeeConfig[configId] != 0
        ) return;
    ...
    function _safeTransfer(address token, address to, uint256 value) internal {
        (bool ok, bytes memory result) =
            token.call(abi.encodeCall(IERC20Like.transfer, (to, value)));
