...
///      engine's entire 4626 balance backs exactly one adapter and `balanceOf(adapter)` is the whole
...
///      the venue, so proceeds never rest here). `decimals`/`asset` are derived from the wrapped
///      vault; `name`/`symbol` use immutable constructor overrides when supplied and otherwise derive
///      from the vault.
contract ERC4626Engine is IVaultEngine, IVaultEngineShares {
    ...
    /// @notice Config owner (bind the core, rotate owner). Holds NO custody power.
